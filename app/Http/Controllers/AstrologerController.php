...
class AstrologerController extends Controller
{
    public function services()
    {
        ...
        $services = AstroService::where('customer_id', $user->id)->orderByDesc('created_at')->get();

        return view('userdashboard-new.astrologer.services.index', compact('services'));
    }
    ...
    {
        $user = Auth::guard('customer')->user();
        $service = AstroService::where('customer_id', $user->id)->findOrFail($id);
    ...
    {
        $user = Auth::guard('customer')->user();
        $consultations = AstroConsultation::where('customer_id', $user->id)->orderByDesc('created_at')->get();
        $services = AstroService::where('customer_id', $user->id)->orderBy('service_name')->get();

        return view('userdashboard-new.astrologer.consultations.index', compact('consultations', 'services'));
    }
    ...
    {
        $user = Auth::guard('customer')->user();
        $consultation = AstroConsultation::where('customer_id', $user->id)->findOrFail($id);
    ...
    {
        $user = Auth::guard('customer')->user();
        $reports = AstroClientData::where('customer_id', $user->id)->orderByDesc('created_at')->get();

        return view('userdashboard-new.astrologer.reports.index', compact('reports'));
    }
    ...
    {
        $user = Auth::guard('customer')->user();
        $report = AstroClientData::where('customer_id', $user->id)->findOrFail($id);
        $report->delete();
    ...
    public function consultationForm(Request $request, $slug)
    {
        ...
        return view('frontend.astrologer.consultation', compact('customer'));
    }
    ...
    public function storePublicConsultation(Request $request, $slug)
    {
        ...
        $consultation = AstroConsultation::create([
            'customer_id' => $customer->id,
            'company_id' => $customer->company_id,
            'client_name' => $request->client_name,
            'client_mobile' => $request->client_mobile,
            'consultation_mode' => $request->consultation_mode ?? 'phone',
            'appointment_date' => $request->appointment_date,
            'appointment_time' => $request->appointment_time,
            'birth_details' => $birthDetails,
            'property_details' => $propertyDetails,
        ...
        return redirect()->route('astro.consultation.thanks', ['slug' => $slug, 'consultationId' => $consultation->id]);
    }

    public function consultationThanks(Request $request, $slug, $consultationId)
    {
        $customer = customer::where('slug', $slug)->firstOrFail();
        $consultation = AstroConsultation::where('customer_id', $customer->id)->findOrFail($consultationId);

        return view('frontend.astrologer.thanks', compact('customer', 'consultation'));
    }
