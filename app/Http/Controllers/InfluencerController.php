...
class InfluencerController extends Controller
{
    public function stats()
    {
        ...
        $stats = CreatorStat::where('customer_id', $user->id)->orderByDesc('created_at')->get();

        return view('userdashboard-new.influencer.stats.index', compact('stats'));
    }
    ...
    {
        $user = Auth::guard('customer')->user();
        $stat = CreatorStat::where('customer_id', $user->id)->findOrFail($id);
    ...
    {
        $user = Auth::guard('customer')->user();
        $collaborations = BrandCollaboration::where('customer_id', $user->id)->orderByDesc('created_at')->get();

        return view('userdashboard-new.influencer.collaborations.index', compact('collaborations'));
    }
    ...
    {
        $user = Auth::guard('customer')->user();
        $collaboration = BrandCollaboration::where('customer_id', $user->id)->findOrFail($id);
    ...
    {
        $user = Auth::guard('customer')->user();
        $portfolio = CreatorPortfolio::where('customer_id', $user->id)->orderByDesc('created_at')->get();

        return view('userdashboard-new.influencer.portfolio.index', compact('portfolio'));
    }
    ...
    {
        $user = Auth::guard('customer')->user();
        $item = CreatorPortfolio::where('customer_id', $user->id)->findOrFail($id);
    ...
    public function collaborationForm(Request $request, $slug)
    {
        $customer = customer::where('slug', $slug)->firstOrFail();

        return view('frontend.influencer.collaboration', compact('customer'));
    }

    public function storePublicCollaboration(Request $request, $slug)
    {
        $customer = customer::where('slug', $slug)->firstOrFail();
        ...
        $request->validate([
            ...
            'brand_mobile' => 'required|string|max:30',
            'collaboration_type' => 'nullable|string|max:60',
            'platform' => 'nullable|string|max:60',
            'budget' => 'nullable|numeric|min:0',
            'campaign_brief' => 'nullable|string',
        ...
        $collaboration = BrandCollaboration::create([
        ...
        return redirect()->route('influencer.collaboration.thanks', ['slug' => $slug, 'collaborationId' => $collaboration->id]);
    }

    public function collaborationThanks(Request $request, $slug, $collaborationId)
    {
        $customer = customer::where('slug', $slug)->firstOrFail();
        $collaboration = BrandCollaboration::where('customer_id', $customer->id)->findOrFail($collaborationId);

        return view('frontend.influencer.thanks', compact('customer', 'collaboration'));
    }
