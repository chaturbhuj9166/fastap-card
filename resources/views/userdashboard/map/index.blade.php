    ...
    <div class="container-fluid them_change_second">
      ...
      <!--</ol>-->
		<div class="box_general padding_bottom">
			...
			$Maps = DB::table('map')->where('uid',$uid)->first();
			@endphp
			<form action="saveMap" method="post" enctype="multipart/form-data">
				 @csrf
			<div class="row">
				<div class="col-md-6">
					<div class="form-group">
						<label>Map iframe</label>
						<textarea type="text" class="form-control" value="{{isset($Maps->map) && $Maps->map !='' ? $Maps->map :''}}" name="map" placeholder="Map Iframe">{{isset($Maps->map) && $Maps->map !='' ? $Maps->map :''}}</textarea>
					<input type="hidden" name="id" value="{{isset($Maps->id) && $Maps->id !='' ? $Maps->id :''}}">
					</div>
					<p style="color:red;">@error('Map'){{$message}}@enderror</p>
				</div>
			</div>
			<!-- /row-->
