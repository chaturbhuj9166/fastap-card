...
@section('page-title', 'Solar AMC Plans')
@section('breadcrumb', 'Solar AMC Plans')
...
<div class="content-page">
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ url('/company/solar/amc') }}" class="form-grid">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="amc_type" class="form-input" placeholder="basic/premium">
                </div>
                <div class="form-group">
    ...
    <div class="card">
        ...
        <div class="card-body">
            @if($amcPlans->count() === 0)
                <p class="empty-state">No AMC plans yet.</p>
            @else
                @foreach($amcPlans as $plan)
                    <div class="item-card">
                        <div class="item-header">
                            <strong>{{ $plan->amc_type ?? 'AMC Plan' }}</strong>
                            <span>{{ $plan->visit_frequency ?? 'n/a' }}</span>
                        </div>
                        <p class="item-desc">Next visit: {{ $plan->next_visit_date?->format('Y-m-d') ?? 'n/a' }}</p>
                        <form method="POST" action="{{ url('/company/solar/amc/' . $plan->id) }}" class="inline-form" onsubmit="return confirm('Delete this AMC plan?');">
                            @csrf
                            @method('DELETE')
