...
@section('breadcrumb', 'Add Portfolio Item')
...
<div class="form-page">
    <div class="page-header fade-up">
        <div class="page-header-content">
            ...
            <p>Showcase your talent with photos, videos, or audio samples</p>
        </div>
    ...
    <form action="{{ route('talent.portfolio.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="form-card fade-up">
            ...
            <div class="form-card-body">
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <select id="talent_type" name="talent_type" class="form-control @error('talent_type') is-invalid @enderror" required>
                            ...
                            <option value="actor" {{ old('talent_type') == 'actor' ? 'selected' : '' }}>Actor</option>
                            <option value="model" {{ old('talent_type') == 'model' ? 'selected' : '' }}>Model</option>
                            <option value="singer" {{ old('talent_type') == 'singer' ? 'selected' : '' }}>Singer</option>
                            <option value="dancer" {{ old('talent_type') == 'dancer' ? 'selected' : '' }}>Dancer</option>
                            <option value="youtuber" {{ old('talent_type') == 'youtuber' ? 'selected' : '' }}>YouTuber</option>
                            <option value="music_producer" {{ old('talent_type') == 'music_producer' ? 'selected' : '' }}>Music Producer</option>
                            <option value="anchor" {{ old('talent_type') == 'anchor' ? 'selected' : '' }}>Anchor</option>
                            <option value="influencer" {{ old('talent_type') == 'influencer' ? 'selected' : '' }}>Influencer</option>
                            <option value="custom" {{ old('talent_type') == 'custom' ? 'selected' : '' }}>Custom Talent</option>
                        </select>
                        @error('talent_type')
                ...
                <div class="form-group">
                    <label for="media_file">Media File <span class="required">*</span></label>
                    ...
                    @error('media_file')
                        <span class="form-error">{{ $message }}</span>
                    @enderror
                    <small class="form-hint">Upload image (JPG, PNG), video (MP4, MOV), or audio (MP3, WAV) file</small>
                </div>
                ...
                <div class="form-group" id="thumbnailGroup" style="display: none;">
                    ...
                    @error('thumbnail')
                        <span class="form-error">{{ $message }}</span>
                    @enderror
                    <small class="form-hint">Upload a thumbnail image for your video</small>
                </div>
