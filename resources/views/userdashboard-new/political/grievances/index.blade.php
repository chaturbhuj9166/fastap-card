...
@section('title', 'Public Grievances')

@section('dashboard-content')
<div class="content-page">
    <div class="content-header">
        <div class="content-header-left">
            <h1 class="content-title">Public Grievances</h1>
            <p class="content-subtitle">Track complaints and resolution status</p>
        </div>
    </div>
    ...
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">Add Grievance</h3>
        </div>
        <div class="card-body">
            <form method="POST" action="{{ url('/user/political/grievances') }}" class="form-grid" enctype="multipart/form-data">
                @csrf
            ...
            @else
                @foreach($grievances as $grievance)
                    <div class="item-card">
                        ...
                        <form method="POST" action="{{ url('/user/political/grievances/' . $grievance->id . '/status') }}" class="form-grid">
                            @csrf
                            <div class="form-group">
                                <label class="form-label">Status</label>
                        ...
                        <form method="POST" action="{{ url('/user/political/grievances/' . $grievance->id) }}" class="inline-form" onsubmit="return confirm('Delete this grievance?');">
                            @csrf
                            @method('DELETE')
