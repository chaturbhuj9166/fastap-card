...
<div class="form-page">
    ...
    <form action="{{ route('real-estate.properties.update', $property->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
            ...
            <div class="form-card-body">
                    ...
                    <div class="form-group">
                        ...
                        <select id="profile_type" name="profile_type" class="form-control @error('profile_type') is-invalid @enderror" required>
                            ...
                            @foreach($activeProfiles ?? [] as $profile)
                                <option value="{{ $profile }}" {{ old('profile_type', $property->profile_type) == $profile ? 'selected' : '' }}>
                                    {{ ucfirst($profile) }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="form-group">
                    ...
                    <textarea id="description" name="description" rows="5"
                              ...
                              placeholder="Describe the property, its features, and benefits..." required>{{ old('description', $property->description) }}</textarea>
                    @error('description')
                        <span class="form-error">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <div class="form-card-body">
                <div class="form-row">
                    ...
                    <div class="form-group" id="rental_price_group" style="display: none;">
                        ...
                        <input type="number" id="rental_price" name="rental_price" value="{{ old('rental_price', $property->rental_price) }}"
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="number" id="area" name="area" value="{{ old('area', $property->area) }}"
                ...
                <div class="form-row">
                    ...
                    <div class="form-group" id="parking_group">
                        ...
                        <input type="number" id="parking" name="parking" value="{{ old('parking', $property->parking) }}"
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <select id="status" name="status" class="form-control @error('status') is-invalid @enderror" required>
                            <option value="ready" {{ old('status', $property->status) == 'ready' ? 'selected' : '' }}>Ready to Move</option>
                            <option value="under_construction" {{ old('status', $property->status) == 'under_construction' ? 'selected' : '' }}>Under Construction</option>
                            <option value="upcoming" {{ old('status', $property->status) == 'upcoming' ? 'selected' : '' }}>Upcoming</option>
                        </select>
                        @error('status')
                            <span class="form-error">{{ $message }}</span>
                        @enderror
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="rera_number" name="rera_number" value="{{ old('rera_number', $property->rera_number) }}"
            ...
            <div class="form-card-body">
                <div class="form-row">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="contact_number" name="contact_number" value="{{ old('contact_number', $property->contact_number) }}"
                ...
                <div class="form-group">
                    ...
                    <textarea id="address" name="address" rows="3"
                              ...
                              placeholder="Enter complete address with landmarks" required>{{ old('address', $property->address) }}</textarea>
                    @error('address')
                        <span class="form-error">{{ $message }}</span>
                    @enderror
                </div>
                ...
                @if(is_array($existingImages) && count($existingImages) > 0)
                    <div class="form-group">
                        ...
                        <div class="current-images-grid">
                            ...
                            <small class="form-hint">Upload new images below to replace existing ones</small>
                        </div>
                @endif

                <div class="form-group">
                    <label for="images">Upload New Images (Max 10)</label>
                    ...
                    @error('images.*')
                        <span class="form-error">{{ $message }}</span>
                    @enderror
                    <small class="form-hint">Leave empty to keep existing images. Upload new images to replace all.</small>
                </div>
        ...
        <div class="form-card fade-up">
            ...
            <div class="form-card-body">
                @php
                    $featuresValue = $property->features ?? [];
                    $featuresArray = is_string($featuresValue) ? json_decode($featuresValue, true) : $featuresValue;
                    $selectedFeatures = old('features', is_array($featuresArray) ? $featuresArray : []);
                @endphp
                <div class="features-grid">
                    <div class="feature-checkbox-item">
                        <input type="checkbox" id="feature_pool" name="features[]" value="Swimming Pool" {{ in_array('Swimming Pool', $selectedFeatures) ? 'checked' : '' }}>
                        <label for="feature_pool">
                            <i class="fas fa-swimming-pool"></i> Swimming Pool
                        </label>
                    </div>
                    <div class="feature-checkbox-item">
                        <input type="checkbox" id="feature_gym" name="features[]" value="Gym" {{ in_array('Gym', $selectedFeatures) ? 'checked' : '' }}>
                        <label for="feature_gym">
                            <i class="fas fa-dumbbell"></i> Gym
                        </label>
                    </div>
                    <div class="feature-checkbox-item">
                        <input type="checkbox" id="feature_garden" name="features[]" value="Garden" {{ in_array('Garden', $selectedFeatures) ? 'checked' : '' }}>
                        <label for="feature_garden">
                            <i class="fas fa-tree"></i> Garden
                        </label>
                    </div>
                    <div class="feature-checkbox-item">
                        <input type="checkbox" id="feature_security" name="features[]" value="24x7 Security" {{ in_array('24x7 Security', $selectedFeatures) ? 'checked' : '' }}>
                        <label for="feature_security">
                            <i class="fas fa-shield-alt"></i> 24x7 Security
                        </label>
                    </div>
                    <div class="feature-checkbox-item">
                        <input type="checkbox" id="feature_parking" name="features[]" value="Covered Parking" {{ in_array('Covered Parking', $selectedFeatures) ? 'checked' : '' }}>
                        <label for="feature_parking">
                            <i class="fas fa-parking"></i> Covered Parking
                        </label>
                    </div>
                    <div class="feature-checkbox-item">
                        <input type="checkbox" id="feature_elevator" name="features[]" value="Elevator" {{ in_array('Elevator', $selectedFeatures) ? 'checked' : '' }}>
                        <label for="feature_elevator">
                            <i class="fas fa-building"></i> Elevator
                        </label>
                    </div>
                    <div class="feature-checkbox-item">
                        <input type="checkbox" id="feature_power" name="features[]" value="Power Backup" {{ in_array('Power Backup', $selectedFeatures) ? 'checked' : '' }}>
                        <label for="feature_power">
                            <i class="fas fa-bolt"></i> Power Backup
                        </label>
                    </div>
                    <div class="feature-checkbox-item">
                        <input type="checkbox" id="feature_clubhouse" name="features[]" value="Clubhouse" {{ in_array('Clubhouse', $selectedFeatures) ? 'checked' : '' }}>
                        <label for="feature_clubhouse">
                            <i class="fas fa-home"></i> Clubhouse
                        </label>
                    </div>
                    <div class="feature-checkbox-item">
                        <input type="checkbox" id="feature_playground" name="features[]" value="Children's Play Area" {{ in_array("Children's Play Area", $selectedFeatures) ? 'checked' : '' }}>
                        <label for="feature_playground">
                            <i class="fas fa-child"></i> Children's Play Area
                        </label>
                    </div>
                    <div class="feature-checkbox-item">
                        <input type="checkbox" id="feature_water" name="features[]" value="Water Supply" {{ in_array('Water Supply', $selectedFeatures) ? 'checked' : '' }}>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <select id="furnishing" name="furnishing" class="form-control @error('furnishing') is-invalid @enderror">
                            ...
                            <option value="semi_furnished" {{ old('furnishing', $property->furnishing) == 'semi_furnished' ? 'selected' : '' }}>Semi-Furnished</option>
                            <option value="fully_furnished" {{ old('furnishing', $property->furnishing) == 'fully_furnished' ? 'selected' : '' }}>Fully Furnished</option>
                        </select>
                        @error('furnishing')
                            <span class="form-error">{{ $message }}</span>
                        @enderror
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" id="available_from" name="available_from" value="{{ old('available_from', $property->available_from) }}"
        ...
        <div class="form-card fade-up">
            ...
            <div class="form-card-body">
                <div class="form-checkboxes-row">
                    ...
                    <div class="form-checkbox">
                        <input type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $property->is_active) ? 'checked' : '' }}>
